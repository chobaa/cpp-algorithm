...
void display(void) {
	...
	for (int i = 0; i < 2; i++) {
		...
		draw_airplane();
		for (int j = 0; j < 4; j++) {
			glm::mat4 ModelMatrix2 = glm::rotate(ModelMatrix, j * 30.0f * TO_RADIAN, glm::vec3(0.0f, 0.0f, 1.0f));
			ModelMatrix2 = glm::translate(ModelMatrix2, glm::vec3(200.0f, 0.0f, 0.0f));
